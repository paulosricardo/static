...
def lista_cartoes():
    comando = select(Cartao).order_by(Cartao.cliente,
Cartao.numero)
...
def cadastra_compra(cartao_id, valor, categoria,
estabelecimento):
    ...
    agora = datetime.now()
    compra = Compra(valor=valor, categoria=categoria,
estabelecimento=estabelecimento, cartao=cartao, data=agora)
